...
import { useDispatch, useSelector } from 'react-redux'; // import 'useDispatch' hook to dispatch new values to reduxs states and 'useSelector' hook to access redux states
...
function Nav() { // create a functional component called 'Nav' to render navigation bar
  ...
  let { userData } = useSelector(state => state.user) // extract 'userData' redux state from 'user' slice

  const handleLogout = async () => { // create a function called 'handleLogout' to log the user out
    try {
      const result = await axios.get(serverUrl + "/api/auth/logout", { withCredentials: true }) // make GET request to this backend URL to log the user out
      // send cookies along with the request for user authentcation so that only authorized users can make requests
      dispatch(setUserData(null)) // set value of 'userData' redux state to null using 'setUserData' reducer
      navigate("/")
      toast.success("LogOut Successfully") // render a toast message that user was logged out successfully
    } catch (error) { // if any error occurs while logging out the user
      console.log(error.response.data.message) // render the error message returned by the backend as a toast/pop-up message
    }
  }
    ...
    <div>
      <div className='w-[100%] h-[70px] fixed top-0 px-[20px] py-[10px] flex items-center justify-between bg-[#00000047]  z-10'>
        <div className='lg:w-[20%] w-[40%] lg:pl-[50px] '>
          <img src={logo} className=' w-[60px]  rounded-[5px] border-2 border-white cursor-pointer' onClick={() => navigate("/")} alt="" /> {/* clicking this image takes user to home page */}
        </div>
